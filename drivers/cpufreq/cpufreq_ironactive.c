 ...
 * Updates: Pranav Vashi <neobuddy89@gmail.com>
...
/* ironactive version */
#define IRONACTIVE_VERSION_MAJOR	(2)
#define IRONACTIVE_VERSION_MINOR	(0)
...
static void cpufreq_ironactive_timer(unsigned long data)
{
	...
	if (new_freq < pcpu->target_freq &&
		...
		trace_cpufreq_ironactive_notyet(data, cpu_load,
...
static void cpufreq_ironactive_idle_end(void)
{
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		cpufreq_ironactive_timer_resched(smp_processor_id(), false);
...
static void cpufreq_ironactive_boost(void)
{
	int i;
	int anyboost = 0;
...
static struct cpufreq_ironactive_tunables *restore_tunables(
						struct cpufreq_policy *policy)
{
	int cpu;
	...
	case CPUFREQ_GOV_LIMITS:
		...
		for_each_cpu(j, policy->cpus) {
			...
			if (policy->min < pcpu->min_freq)
				cpufreq_ironactive_timer_resched(j, true);
			pcpu->min_freq = policy->min;
